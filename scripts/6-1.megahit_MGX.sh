...
# This script is for assembling each subject's metagenomic datasets into contigs.
...
OUTPUT_DIR=${BASE}/intermediates/6.megahit/MGX
mkdir -p ${OUTPUT_DIR}
...
cd ${INPUT_DIR}
sed '1d' ${INPUT_LIST} | awk '{print $1}' | sort | uniq | sed -n "${BEGIN}, ${END}p" | while read subject
do
	echo -------------------------------------------
	echo ${subject}
	...
	#echo ${pe2}
	#echo ${se}
	megahit --out-dir ${OUTPUT_DIR}/${subject} --out-prefix ${subject}_MGX --num-cpu-threads 36 --memory 0.3 --presets meta-sensitive -1 ${pe1} -2 ${pe2} -r ${se}
	sed -i -e "s/>/>${subject}_MGX_/g" -e "s/ /_/g" ${OUTPUT_DIR}/${subject}/${subject}_MGX.contigs.fa
