...
############################################################################################################################################
# Script: ~/crisprome/hmp/scripts/3.fastp_MTX.sh
...
# Set parameters.
BASE=~/crisprome/hmp
INPUT_DIR=~/public_raw_data/hmp2/IBDMDB/nonIBD/nat_microbiol_MTX
INPUT_LIST=~/crisprome/hmp/intermediates/1.dataset_summary/1-7.Nat_Microbiol_nonIBD_metadata.txt
OUTPUT_DIR=${BASE}/intermediates/3.fastp/nat_microbiol_MTX
...
do
	...
	do
		...
		if [ ${file_number} = 1 ]; then
			...
			fastp -i ${in} -o ${out} --trim_poly_g --thread 16 -j ${json} -h ${html} --trim_front1 5
		else
			...
			fastp -i ${in1} -I ${in2} -o ${out1} -O ${out2} --unpaired1 ${unpaired} --unpaired2 ${unpaired} -j ${json} -h ${html} --correction --trim_poly_g --detect_adapter_for_pe --thread 16 --trim_front1 5 --trim_front2 5
		fi
	done
